        ...
        success: function(respuesta) {
            ...
            $.ajax({
                ...
                success: function(datos){
                    ...
                    var falso = 0;
                    var confi = 0;
                    for(var i =0; i < datos.respuesta.length; i++) {
                        if(datos.respuesta[i].estado == esta) {
                            confi = datos.respuesta[i].confirmados;
                            sospe = datos.respuesta[i].sospechosos;
                            falso = datos.respuesta[i].falsos;
                        }
                    }
                    var position = null;
                    switch (esta) {
                    ...
                    '<div id="casos">' +
                        '<div class="danger">' +
                            '<div style="background:red;">'+confi+'</div>' +
                            ' Casos confirmados' +
                        '</div>' +
                        '<div class="danger">' +
                            '<div style="background:orange;">'+sospe+'</div>'+
                            ' Casos sospechosos' +
                        '</div>' +
                        '<div class="danger">' +
                            '<div style="background:green;">'+falso+'</div>' +
                            ' Casos falsos' +
                        '</div>'
                    '</div>';
                ...
                success: function(datos){
                    ...
                    $.each(estados, function(index, value) {
                        ...
                        var bermudaTriangle = new google.maps.Polygon({
                            ...
                            fillOpacity: 0.80,
                            estado: index,
                            confirma: datos.respuesta[j].confirmados,
                            sospechoso: datos.respuesta[j].sospechosos,
                            falsos: datos.respuesta[j].falsos      
                       });
                       j++;
                        ...
                        switch (index) {
                            ...
                            var contentString = '<h1 id="firstHeading" class="firstHeading">Casos de Covid-19 en '+estado+'</h1>'+ 
                                '<div id="casos">' +
                                    '<div class="danger">' +
                                        '<div style="background:red;">' + this.confirma+ '</div>' +
                                        'Casos confirmados' +
                                    '</div>' +
                                    '<div class="danger">' +
                                        '<div style="background:orange;">' + this.sospechoso+ '</div>'+
                                        'Casos sospechosos' +
                                    '</div>' +
                                    '<div class="danger">' +
                                        '<div style="background:green;">' + this.falsos+ '</div>' +
                                        'Casos falsos' +
                                    '</div>'
                                '</div>';
